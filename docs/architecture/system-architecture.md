...
**Database Design 2.2, implemented by the Phase 8B revision (2026-09-06)**: there is no `carts` or `cart_items` MySQL table. Cart state is ephemeral and lives entirely outside MySQL:

- **Guest customers**: cart lives in the browser (`localStorage`, key `ai_commerce.cart.{storeId}`). Never sent to or stored by the backend until checkout, or until a subsequent login triggers a merge (below). Fully client-controlled — see "Untrusted input" below.
- **Authenticated customers**: cart lives in Redis, behind `GET/POST/PATCH/DELETE /api/cart*` (`App\Http\Controllers\Customer\CartController` → `App\Services\CartService` — the controller never touches Redis directly). The key (`cart:{store_id}:{customer_id}`) is **derived server-side** from `CustomerContext` (resolved by the pre-existing `ResolveCustomerContext` middleware) — **never** from client-supplied `organization_id`/`store_id`/`customer_id`, matching the same discipline already required for `TenantContext` elsewhere (§2, §9). The key carries a **sliding TTL**, reissued on every mutation (`config('cart.ttl_seconds')`, `CART_TTL_SECONDS` — no prior convention fixed this value, so a 30-day provisional default was chosen at implementation time; see `backend/config/cart.php`). The connection is its own Redis logical database (`config('cart.redis_connection')`, default `cart`, `REDIS_CART_DB`) — deliberately never the queue's `default` connection or the analytics-cache `cache` connection, keeping all four Redis roles (§1) operationally isolated from each other.
- **Concurrency primitive**: a Redis **Hash** per cart, field = `product_variant_id`, value = quantity, mutated via atomic `HINCRBY` (add/merge) or `HSET`/`HDEL` (set-exact-quantity/remove). This gives the same no-lost-update guarantee the old MySQL `unique(cart_id, product_variant_id)` + upsert pattern provided, without the read-modify-write race a single JSON-blob cart would introduce.
- **Redis stores the minimum only** — `{product_variant_id: quantity}`, nothing else. No price, name, SKU, or inventory is ever written to Redis; every read (`GET /api/cart`, and every mutation's response) hydrates display data fresh from MySQL, scoped to the customer's own store and to currently-Active variants — the same "never trust a stored snapshot" principle already applied to every Eloquent API Resource in this codebase. A variant id that no longer resolves (deleted, archived, or otherwise invalid) is dropped from the response and lazily pruned from the Redis hash — one stale line never fails the whole cart read.
- **Merge-on-login, resolved**: when a guest with a non-empty `localStorage` cart authenticates (login or registration — not a session restore from an existing valid token), the frontend calls `POST /api/cart/merge` once with the guest cart's `{product_variant_id, quantity}` lines. The server **sums** each line into whatever already exists in the authenticated Redis cart via the same atomic `HINCRBY` a plain add uses — guest `A×2` merging into an existing Redis `A×3` always yields `A×5`, order-independent. The `localStorage` cart is cleared **only after** the merge call succeeds; a failed merge leaves it untouched, surfaces a recoverable error, and never affects authentication itself. An empty guest cart at login time makes no merge call at all — the authenticated cart simply loads normally.
- **Logout does not delete the Redis cart.** It remains exactly as it was, available again on the next login (its own merge, if any, running again against it). The browser simply stops treating it as active and falls back to a fresh (typically empty, since the prior merge already cleared it) guest `localStorage` cart — never a client-side copy of the Redis cart.
- **Untrusted input, unchanged**: whether from `localStorage` or Redis, Laravel treats cart contents as untrusted input at checkout. Only `product_variant_id` and quantity are read from it — price, name, availability, and totals are never accepted from the client (see §9's existing "order totals always server-side" rule, which this reinforces rather than changes) and are always looked up/recalculated fresh from MySQL. Checkout (`CheckoutController`/`CheckoutOrderCreationService`) is untouched by this revision — it already read only `{product_variant_id, quantity}` from whatever cart source the client sent, so nothing about its own validation needed to change.
- **Inventory is never enforced by the cart layer**, deliberately — a cart (guest or authenticated) may hold a quantity greater than what's currently in stock, including for a variant with zero inventory. This is consistent with the MVP's "no soft-hold/reservation system" decision (§9): checkout's own locked `InventoryAdjustmentService` claim remains the sole authoritative inventory gate, exactly as it already was before this revision.
...
Full schema-level rationale: `../database/database-design.md` §"Cart — intentionally NOT a MySQL table." Full implementation record, including the real architectural deviation this revision closes (Block 8B originally shipped a single `localStorage` cart shared by guest and authenticated customers alike, contradicting this section as it stood at the time) and its verification: `docs/development/project-status.md`, "Phase 8 Block 8B Revision" entry.
