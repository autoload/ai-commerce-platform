...
export function CustomerLoginPage() {
  ...
  const { login, isLoggingIn, loginError, status } = useCustomerAuth()
  ...
  if (status === 'authenticated') {
    const redirectTo = (location.state as LocationState | null)?.from?.pathname ?? `/store/${params.storeId}`
    ...
    <div className="flex min-h-svh items-center justify-center bg-slate-50 px-4 dark:bg-slate-950">
      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <Link
            to={`/store/${params.storeId}/register`}
