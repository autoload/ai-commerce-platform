...
export function CustomerRegisterPage() {
  ...
  const { register, isRegistering, registerError, status } = useCustomerAuth()
  ...
  if (status === 'authenticated') {
    return <Navigate to={`/store/${params.storeId}`} replace />
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(`/store/${params.storeId}`, { replace: true })
    } catch {
  ...
  return (
    <div className="flex min-h-svh items-center justify-center bg-slate-50 px-4 py-8 dark:bg-slate-950">
      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Create an account with this store.</p>
